...
from src.fl.client_data import ClientDataSource
...
def _create_model_like(
    global_model: nn.Module,
) -> nn.Module:
    """
    Create a fresh model with the same architecture as the
    global model.
    """
...
def run_federated_round(
    ...
    The same coordinator can operate on different client
    partitioning strategies through `client_data_source`.

    If no client data source is supplied, the original
    device-based N-BaIoT client definition is used.
    ...
    if not client_ids:
        raise ValueError(
            "At least one client_id is required."
        )
    ...
    for client_id in client_ids:
        local_model = _create_model_like(
            global_model
        )
