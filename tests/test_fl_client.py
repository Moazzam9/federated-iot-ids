...
def training_scaler(
    ...
    The scaler must already exist because this test is intended to
    exercise the real preprocessing pipeline rather than create a
    new preprocessing artifact.
    """
    scaler_path = (
        PROJECT_ROOT
        / "data"
...
def _expected_device_training_samples(
    ...
    This uses the validated source-index metadata. It does not scan
    the raw CSV files again.
    """
    total = sum(
        record.train_count
...
def test_client_sample_count_uses_validated_metadata(
    ...
    """
    Verify that client sample counting comes directly from the
    validated source-index metadata.

    This is deliberately a metadata-only test. It protects the
    performance fix that prevents train_client() from rescanning
    every raw CSV row merely to determine FedAvg weighting.
    """
    ...
    actual_samples = _count_client_training_samples(
        loader=nbaiot_loader,
        client_id=DEVICE,
    )

    assert actual_samples == expected_samples
    assert actual_samples == 712_809
...
def test_client_sample_count_rejects_unknown_client(
    ...
    """
    Verify that an unknown client cannot silently receive a zero
    FedAvg weight.
    """
    with pytest.raises(
        ValueError,
...
def test_real_nbaiot_client_trains_one_small_batch(
    ...
    remains a smoke test rather than a full training experiment.

    Important:
    `result.samples` represents the client's complete training
    dataset size. It is used for FedAvg weighting and therefore
    must NOT equal the size of the single smoke-test batch.
    """
